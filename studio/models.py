...
class StudioImage(models.Model):
    ...
    # كان الحقل ده موجود في migration (0005) بس اتشال بالغلط من هنا (الموديل)
    # من غير ما تتعمل migration لحذف العمود من قاعدة البيانات — يعني الموديل
    # والداتابيز كانوا مش متزامنين. استعادته هنا (بدل حذفه من الداتابيز) عشان:
    # (1) نتجنب فقدان أي بيانات لو العمود اتملى فعليًا في بعض الصفوف،
    # (2) templates/landing.html بيعتمد عليه فعلاً (hero_image/banner_1/banner_2)
    # لعرض نسخة مضغوطة بدل الصورة الكاملة — من غيره كان بيرجع دايمًا لـ
    # fallback (الصورة الكاملة) بصمت بلا أي خطأ ظاهر.
    web_image = models.ImageField(
        upload_to='studio/web/%Y/%m/',
        blank=True,
        null=True,
        verbose_name='صورة العرض المضغوطة',
    )
