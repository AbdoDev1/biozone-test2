...
def parse_unit_row(row_num, row, idx, account_types_by_col):
    ...
    # ملاحظة: مبنتحققش هنا إن category_slug ده قسم موجود فعلًا — لو القيمة
    # لسه مش موجودة في القاعدة، هتتعمل تلقائيًا وقت الحفظ الفعلي (راجع
    # get_or_create_category في commit.py). مرحلة القراءة دي "بدون حفظ"
    # فمش بننشئ القسم هنا، بس مبنرفضش الصف عشانه برضه.
